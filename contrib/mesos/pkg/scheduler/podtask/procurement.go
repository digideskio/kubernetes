...
type BuildStaticPodConfig func(host string) ([]byte, error)

// StaticPodProcurement assigns static pod configuration to a task's ExecutorInfo.Data
// field; the configuration is generated by the dataGenerator func. This implementation
// caches generated configs on a per-host basis to save cycles.
func StaticPodProcurement(dataGenerator BuildStaticPodConfig) Procurement {
	cache := struct {
		sync.Mutex
	...
	get := func(k string) ([]byte, error) {
		cache.Lock()
		defer cache.Unlock()
		v, ok := cache.entries[k]
		if ok {
			return v, nil
		}

		v, err := dataGenerator(k)
		if err != nil {
			return nil, err
		}
	...
	return func(t *T, offer *mesos.Offer) error {
