...
func CreateOrUpdate(client *client.Client, hostName string, labels map[string]string) (*api.Node, error) {
	...
	if !errors.IsAlreadyExists(err) {
		return nil, fmt.Errorf("unable to register %s with the apiserver: %v", hostName, err)
	}
...
// IsSlaveAttributeLabel returns iff the given label is derived from a slave attribute
